=head1 ActivePerl 820 -- Release Notes
...
check L<perl58delta/Incompatible Changes> for known source level
incompatibilities between the Perl 5.8 releases and the earlier releases
in the Perl 5.6 series.

You may also want to check L<perl581delta/Incompatible Changes>,
...
are now installed in F<$PREFIX/lib>.  In ActivePerl build 817 and
earlier, these were installed in F<$PREFIX/site/lib>.

The $Config{siteprefix} is now F<$PREFIX/site>.  In ActivePerl build
817 and earlier, it was just F<$PREFIX> and $Config{sitelib}
...
get installed in F<$PREFIX/site/bin>.  In ActivePerl build 817 and
earlier, these were installed in F<$PREFIX/bin>.  Another consequence of
the $Config{siteprefix} update is that modules configured and built
with C<perl Makefile.PL PREFIX=...> are now installed directly in the
...
C<ppm profile>, C<ppm properties>, C<ppm set>, and C<ppm target>
...
you should reconfigure them manually with C<ppm repo add ...> or in the
...
The "Upgradable View" shows packages that would downgrade the
installation. PPM will warn you if select a package for installation
which would downgrade the currently installed version.
